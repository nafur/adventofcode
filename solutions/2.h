...
struct solve<2> {
	static auto solution() {
		return std::make_pair(44670,285);
	}
	
	const std::string input = R"AB(
...
2257	155	734	228	204	2180	175	2277	180	2275	2239	2331	2278	1763	112	2054
	...
	int first(const std::vector<std::vector<int>>& data) const {
		return std::accumulate(data.begin(), data.end(), 0l,
			[](int sum, const std::vector<int>& line) {
				auto ext = std::accumulate(line.begin(), line.end(), std::make_pair(10000l,0l),
					[](const std::pair<int,int>& ext, int val){
						//std::cout << "Merging " << val << std::endl;
						return std::make_pair(std::min(val,ext.first), std::max(val,ext.second));
					}
				);
				return sum + (ext.second - ext.first);
			}
		);
	}
	
	int second(const std::vector<std::vector<int>>& data) const {
		int sum = 0;
		for (const auto& line: data) {
			bool found = false;
			for (auto val: line) {
				auto it = std::find_if(line.begin(), line.end(), [val](auto v){
					return (val != v) && (val % v == 0);
				});
				if (it != line.end()) {
					found = true;
					sum += (val / *it);
	...
	auto operator()() const {
		std::vector<std::vector<int>> data = parse_table(input);
		return std::make_pair(first(data), second(data));
	}
};
